...
| 画面 | 役割 |
| --- | --- |
| 初回設定 | 区・地区を選ぶと曜日が自動で決まる設計だが、画面は未対応で今は曜日を手入力する暫定動作。設定するまで他の画面に進まない |
...
**本来は区→地区を選ぶだけで曜日が自動的に確定する**のが目標形。

`assets/data/areas.json` の `areas` には、さいたま市「収集日カレンダー」から取得した
77件の確定地区（区分ごとの収集曜日・早朝収集地区かどうかを含む）が入っている
（`scripts/update_areas_json.mjs`で再取得できる。詳細は
[docs/next-phase.md](docs/next-phase.md) A章）。ただし`AreaEditorPage`（初回設定・地区変更の
画面）はまだこのデータを使わず曜日の手入力を主UIとして描いており、**データはあるのに
画面が追いついていない**のが現状（[docs/requirements.md](docs/requirements.md) 4.1節）。

```jsonc
{
  "presets": [ /* 曜日入力の出発点となる雛形。地区が見つからない代替経路でのみ使う */ ],
  "areas":   [ /* 町丁目から確定できる地区。77件 */ ]
}
```

`AreaCatalog.areasInWard()` を使った「区を選ぶ → 地区を選ぶ」の導線は
アプリ側のコードを変えずに載る設計・テスト済みなので、残る作業は
`AreaEditorPage`側だけ（次の一歩）。
...
[docs/next-phase.md](docs/next-phase.md) を参照。市の地区表の取り込みは初回設定の主経路を
機能させるための前提条件で、現状のボトルネック。収集日前夜のローカル通知はそれとは独立に
着手できる追加機能。粗大ごみ判定・「これは出せるか」をAIに聞く機能はファーストリリース対象外。
