...
| 画面 | 役割 |
| --- | --- |
| 初回設定 | 区・地区を選ぶと曜日が自動で決まる（地区データが揃うまでは曜日を手入力する暫定動作）。設定するまで他の画面に進まない |
...
**本来は区→地区を選ぶだけで曜日が自動的に確定する**のが目標形だが、
**このリポジトリには、町丁目から地区を引くための市の地区表がまだ入っていない**ため、
現状は代わりに利用者が自分で収集曜日を登録する暫定動作になっている
（詳しい経緯は[docs/requirements.md](docs/requirements.md) 4.1節・[docs/next-phase.md](docs/next-phase.md) A章）。
